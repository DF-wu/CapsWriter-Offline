...
class FastWhisperMel:
    ...
    def __init__(
        ...
        if filter_path and os.path.exists(filter_path):
            ...
            self.filters = self._generate_filters(
                sr, n_fft, n_mels, f_min, f_max, norm, mel_scale
            )
    ...
    def _generate_filters(self, sr, n_fft, n_mels, f_min, f_max, norm, mel_scale):
        ...
        m_pts = np.linspace(
            hz_to_mel(f_min, mel_scale), hz_to_mel(f_max, mel_scale), n_mels + 2
        )
...
class QwenAudioEncoder:
    ...
    def __init__(
        ...
        _prepare_onnx_cuda_runtime(use_cuda)
        ...
        elif use_dml and "DmlExecutionProvider" in available_providers:
            providers.insert(0, "DmlExecutionProvider")
            self.active_dml = True

        if self.verbose:
            print(
                f"--- [Encoder] 加载 Split ONNX 模型 (DML: {self.active_dml}, Pad: {pad_to}s) ---"
            )
        ...
        # 加载两个 Session
        try:
            self.sess_fe = ort.InferenceSession(
                frontend_path, sess_options=sess_opts, providers=providers
            )
            self.sess_be = ort.InferenceSession(
                backend_path, sess_options=sess_opts, providers=providers
            )
        except Exception:
            self.active_cuda = False
            self.active_dml = False
            ...
            self.sess_be = ort.InferenceSession(
                backend_path, sess_options=sess_opts, providers=["CPUExecutionProvider"]
            )
        ...
        # 预热处理
        try:
            if self.pad_to > 0 and (self.active_dml or self.active_cuda):
                if self.verbose:
                    print(f"--- [Encoder] 正在预热 (固定形状: {self.pad_to}s)... ---")
                dummy_wav = np.zeros(int(16000 * self.pad_to)).astype(np.float32)
                _ = self.encode(dummy_wav)
            else:
                # 非 GPU/DML 模式下，预热一个短音频即可，无需 Padding
                if self.verbose:
                    print(f"--- [Encoder] 正在预热 (非 GPU/DML 模式)... ---")
                dummy_wav = np.zeros(int(16000 * 2.0)).astype(np.float32)
                _ = self.encode(dummy_wav)
        except Exception:
            if self.active_cuda or self.active_dml:
                self.active_cuda = False
                self.active_dml = False
                self.sess_fe = ort.InferenceSession(
                    frontend_path,
                ...
                if self.verbose:
                    print(
                        "--- [Encoder] GPU/DML 预热失败，回退 CPUExecutionProvider ---"
                    )
                dummy_wav = np.zeros(int(16000 * 2.0)).astype(np.float32)
                _ = self.encode(dummy_wav)
            else:
    ...
    def _run_backend(self, hidden_states: np.ndarray) -> np.ndarray:
        ...
        if self.active_dml and seq_len < self.h_target_len:
            ...
            hidden_input = np.pad(
                hidden_states, ((0, 0), (0, pad_width), (0, 0)), mode="constant"
            )

            # 构造 Mask：前 seq_len 为 0 (关注)，后 pad_width 为 -10000.0 (屏蔽)
            # 维度需要广播到 (Batch, 1, T_fixed, T_fixed)
            mask = np.zeros(
                (batch, 1, self.h_target_len, self.h_target_len), dtype=self.input_dtype
            )
        ...
        # 2. 执行推理
        audio_embd = self.sess_be.run(
            None, {"hidden_states": hidden_input, "attention_mask": mask}
        )[0]
